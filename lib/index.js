...
class CaltrainApi {
    ...
    _location_from_cenroid(centroid) {
    ...
    async stop_places() {
        return await (request(this.api_stop_places()).then(function(response) {
            ...
            return places.map(function(server_stop) {
                return new StopPlace(server_stop['@id'], server_stop.Name, this._location_from_cenroid(server_stop.Centroid), server_stop.PublicCode, this._heading(server_stop.quays));
            });
        }));
    }
}
